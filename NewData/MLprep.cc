...
void MLprep(){
    ...
    TFile *f_background = new TFile("NewData/ROOT_files/background.root", "RECREATE");
    TTree *t_background = new TTree("Tback", "Tree with selected variables from experimental data");

    TFile *f_signal = new TFile("NewData/ROOT_files/signal.root", "RECREATE");
    ...
    for(Long64_t i = 0; i < nEntries_data; i++){    
        ...
        if (mass_b < s_left || mass_b > s_right){
            ...
            if (vars_data["bPt"] != 0){
                ...
                vars_background["kstTrkpPtR"] = vars_data["kstTrkpPt"] / vars_data["bPt"];
                vars_background["muLeadingPtR"] = muLeadingPt / vars_data["bPt"];
                vars_background["muTrailingPtR"] = muTrailingPt / vars_data["bPt"];
    ...
    for(Long64_t i = 0; i < nEntries_mc; i++){    
        ...
        if (mass_b > s_left && mass_b < s_right){
            ...
            if (vars_mc["bPt"] != 0){
                ...
                vars_signal["kstTrkpPtR"] = vars_mc["kstTrkpPt"] / vars_mc["bPt"];
                vars_signal["muLeadingPtR"] = muLeadingPt / vars_mc["bPt"];
                vars_signal["muTrailingPtR"] = muTrailingPt / vars_mc["bPt"];
