...
void plot_final(){
    ...
    // taken from evaluation results
    double threshold = 0.81563126252505;

    // Create output directory if it doesn't exist
    fs::create_directories("NewData/Final_Plots/Data");
    //fs::create_directories("NewData/Final_Plots/MC");

    // --- Open Files and Get Trees ---
    TFile *f = new TFile("NewData/ROOT_files/data_selected_ml_output.root", "read");
    //TFile *f = new TFile("NewData/ROOT_files/mc_selected_ml_output.root", "read");
    ...
    // --- Connect score and eventN branches ---
    Float_t score;
    Long64_t eventN;
    t->SetBranchAddress("MLscore", &score);
    ...
    for (const auto &name : variables){
        ...
        h[name] = new TH1D((name + "_h").c_str(), "", bins, xmin, xmax);
        h_cut[name] = new TH1D((name + "_h_cut").c_str(), "", bins, xmin, xmax);
    ...
    // --- Only keep candidate with higher MLscore per event ---
    unordered_map<Long64_t, pair<Float_t, Long64_t>> bestPerEvent; 
    // key   = eventN (unique event number)
    // value = (bestScore, entryIndex) for that event

    for (Long64_t i = 0; i < nEntries; ++i) {
        t->GetEntry(i); // load candidate i, gives you eventN and score

        auto it = bestPerEvent.find(eventN);

        // If this is the first time we see this eventN, or this candidate
        // has a higher MLscore than the one stored so far → update
        if (it == bestPerEvent.end() || score > it->second.first) {
            bestPerEvent[eventN] = {score, i};
        }
    }
   
    // --- Fill Histograms ---
    for (const auto &kv : bestPerEvent) {
        Long64_t entry = kv.second.second;
        Float_t bestScore = kv.second.first;
    ...
    for (const auto &name : variables){
        ...
        c->SaveAs(Form("NewData/Final_Plots/Data/%s.pdf", name.c_str()));
        //c->SaveAs(Form("NewData/Final_Plots/MC/%s.pdf", name.c_str()));
